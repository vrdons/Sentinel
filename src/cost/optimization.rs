...
use crate::storage::db::{Database, CostOptimization};
...
impl CostOptimizer {
    ...
    fn initialize_fast_suggestions(&self) {
        ...
        let simple_optimizations = vec![
            ...
            ("gpt-4o", "gpt-4o-mini", 85.0, "Cost optimization for basic query"),
            ("gpt-4-turbo", "gpt-4o-mini", 90.0, "Significant cost reduction"),
            ("claude-opus-4.6", "claude-haiku-4.5", 80.0, "Claude tier downgrade"),
            ("claude-sonnet-4.6", "claude-haiku-4.5", 70.0, "Claude simple task optimization"),
            ("o1", "gpt-4o-mini", 98.0, "Reasoning model unnecessary for simple task"),
        ...
        for (original, suggested, savings, reason) in simple_optimizations {
            ...
            cache.insert(key, OptimizationSuggestion {
        ...
        let medium_optimizations = vec![
            ("gpt-4", "gpt-4o", 60.0, "Modern model with better efficiency"),
            ("claude-opus-4.6", "claude-sonnet-4.6", 40.0, "Balanced capability/cost"),
        ];
    ...
    pub async fn analyze_request(&self, request: &ChatRequest, endpoint: &str) -> Result<Option<OptimizationSuggestion>> {
    ...
    fn has_complex_keywords(&self, content: &str) -> bool {
        let complex_keywords = ["explain", "analyze", "design", "implement", "algorithm", "strategy"];
        let content_lower = content.to_lowercase();
        complex_keywords.iter().any(|&keyword| content_lower.contains(keyword))
    }
    ...
    async fn compute_suggestion_slow(&self, request: &ChatRequest) -> Result<Option<OptimizationSuggestion>> {
        ...
        match complexity_score {
            score if score < 0.3 => {
                self.suggest_cheaper_model(current_model, complexity_score).await
            },
            score if score > 0.8 => {
                self.suggest_better_model(current_model, complexity_score).await
            },
            _ => {
                self.suggest_balanced_model(current_model, complexity_score).await
            }
        }
    }

    /// Calculate actual savings after a request is completed
    pub fn calculate_savings(&self, original_model: &str, used_model: &str, tokens: (u32, u32)) -> f64 {
        let original_cost = self.pricing.calculate_cost(original_model, tokens.0, tokens.1);
    ...
    async fn maybe_update_performance_cache(&self) -> Result<()> {
        ...
        if let Some(models) = stats.get("models").and_then(|m| m.as_array()) {
            for model_data in models {
                if let (Some(model), Some(requests), Some(avg_latency), Some(avg_cost), Some(avg_quality)) = (
                    ...
                    new_cache.insert(model.to_string(), ModelPerformance {
    ...
    pub fn assess_complexity(&self, request: &ChatRequest) -> f32 {
        ...
        // Content analysis
        let combined_content = request.messages.iter()
        ...
        let complex_keywords = [
            ...
            "code", "programming", "algorithm", "math", "calculation", "proof",
            "research", "essay", "report", "summary", "translation", "creative"
        ];
    ...
    async fn suggest_cheaper_model(&self, current_model: &str, _complexity: f32) -> Result<Option<OptimizationSuggestion>> {
        // Define model hierarchy (most expensive to least expensive)
        let cheaper_alternatives = match current_model {
            m if m.contains("gpt-4o") && !m.contains("mini") => vec!["gpt-4o-mini", "gpt-3.5-turbo"],
            m if m.contains("gpt-4") => vec!["gpt-4o-mini", "gpt-3.5-turbo"],
            m if m.contains("claude-3-opus") => vec!["claude-3-5-sonnet-20241022", "claude-3-5-haiku-20241022"],
            m if m.contains("claude-3-sonnet") || m.contains("claude-3-5-sonnet") => vec!["claude-3-5-haiku-20241022"],
        ...
        for alternative in cheaper_alternatives {
            ...
            if alt_cost < original_cost {
                ...
                if savings_percent > 20.0 { // Only suggest if savings > 20%
                    return Ok(Some(OptimizationSuggestion {
                        ...
                        confidence_score: 0.8,
                        reason: format!("Simple request detected. {} can handle this for {:.0}% less cost", alternative, savings_percent),
                        quality_impact,
                    }));
                }
            }
    ...
    async fn suggest_better_model(&self, current_model: &str, _complexity: f32) -> Result<Option<OptimizationSuggestion>> {
        ...
        for alternative in better_alternatives {
            ...
            if quality_impact > 0.2 { // Only suggest if significant quality improvement
                return Ok(Some(OptimizationSuggestion {
                    original_model: current_model.to_string(),
                    suggested_model: alternative.to_string(),
                    potential_savings_percent: 0.0, // This is about quality, not cost
                    confidence_score: 0.7,
                    reason: format!("Complex request detected. {} provides better reasoning for complex tasks", alternative),
                    quality_impact,
                }));
            }
        }
    ...
    async fn suggest_balanced_model(&self, current_model: &str, _complexity: f32) -> Result<Option<OptimizationSuggestion>> {
        // For medium complexity, look for the best quality/cost ratio
        let alternatives = vec![
            "gpt-4o-mini", "claude-3-5-haiku-20241022", "gemini-1.5-flash", 
            "mistral-small-latest", "command-r"
        ];
        ...
        for alternative in alternatives {
            ...
            if let (Some(current_perf), Some(alt_perf)) = (current_performance, alt_performance) {
                ...
                let current_qpd = if current_cost > 0.0 { current_perf.avg_quality as f64 / current_cost } else { 0.0 };
                let alt_qpd = if alt_cost > 0.0 { alt_perf.avg_quality as f64 / alt_cost } else { 0.0 };
                ...
                if alt_qpd > current_qpd && alt_qpd > best_score {
                    ...
                    best_alternative = Some(OptimizationSuggestion {
                        ...
                        reason: format!("{} offers better quality per dollar for medium complexity tasks", alternative),
    ...
    fn estimate_quality_impact(&self, from_model: &str, to_model: &str) -> f32 {
        ...
        let model_tiers = [
            ("gpt-4o", 0.9), ("claude-3-opus-20240229", 0.9), ("claude-3-5-sonnet-20241022", 0.85),
            ("gpt-4o-mini", 0.8), ("gemini-1.5-pro", 0.8), ("mistral-large-latest", 0.75),
            ("claude-3-5-haiku-20241022", 0.7), ("gemini-1.5-flash", 0.7), ("gpt-3.5-turbo", 0.6),
            ("command-r", 0.6), ("mistral-small-latest", 0.5)
        ];
        
        let from_quality = model_tiers.iter()
            .find(|(model, _)| from_model.contains(model))
            .map(|(_, quality)| *quality)
            .unwrap_or(0.5);
            
        let to_quality = model_tiers.iter()
    ...
    fn is_already_optimized(&self, model_name: &str) -> bool {
        ...
        match model_name {
            "gpt-4o-mini" | "claude-3-5-haiku-20241022" | "gemini-1.5-flash" | 
            "mistral-small-latest" | "command-r" => true,
...
mod tests {
    ...
    async fn test_complexity_assessment() {
        ...
        let optimizer = CostOptimizer::new(pricing, Arc::new(Database::new(&std::path::PathBuf::from(":memory:")).await.unwrap()));
