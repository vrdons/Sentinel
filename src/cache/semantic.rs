...
impl LocalEmbeddingProvider {
    pub async fn new(model_name: String) -> Result<Self> {
        ...
        let api = Api::new()?;
        let repo = api.repo(Repo::with_revision(repo_id.to_string(), hf_hub::RepoType::Model, revision.to_string()));
        ...
        info!("Loaded model {} ({} dims) in {:?}", model_name, dimension, start.elapsed());
...
impl EmbeddingProvider for LocalEmbeddingProvider {
    async fn embed(&self, text: &str) -> Result<Vec<f32>> {
        ...
        let embeddings = self.model.forward(&token_ids_tensor, &token_type_ids_tensor, Some(&attention_mask_tensor))?;
        ...
        debug!("Generated embedding for '{}...' in {:?}", &text[..text.len().min(20)], start.elapsed());
        Ok(v)
    }
...
impl SemanticCache {
    ...
    async fn setup_database(&self) -> Result<()> {
        ...
        info!("Semantic cache database initialized with {} dimensions", 
              self.embedding_provider.dimension());
        Ok(())
    }
    ...
    fn generate_prompt_key(&self, request: &ChatRequest) -> String {
        ...
        let combined_prompt = request.messages
            .iter()
    ...
    pub async fn get_similar(&self, request: &ChatRequest) -> Result<Option<CachedResponse>> {
        ...
        let query_embedding_bytes: Vec<u8> = query_embedding.iter().flat_map(|f| f.to_le_bytes()).collect();
        
        // Use libsql's vector similarity search if available
        let cutoff_time = chrono::Utc::now() - chrono::Duration::hours(self.config.ttl_hours as i64);
        ...
        let mut rows = match self.db.query(
            "SELECT id, response, timestamp, model, embedding
             ...
             LIMIT 1",
            libsql::params![cutoff_time.to_rfc3339(), request.model.clone(), query_embedding_bytes.clone()],
        ).await {
            Ok(rows) => rows,
            Err(e) => {
                debug!("Native vector search failed, falling back to manual scan: {}", e);
                // Fallback to manual scan if vector_distance is not supported
                return self.get_similar_fallback(request, query_embedding, cutoff_time).await;
            }
        };

        if let Some(row) = rows.next().await? {
            let cached_embedding: Vec<f32> = row.get::<Vec<u8>>(4)?
                .chunks_exact(4)
            ...
            if similarity >= self.config.similarity_threshold {
                ...
                let timestamp = chrono::DateTime::parse_from_rfc3339(&row.get::<String>(2)?)?.with_timezone(&chrono::Utc);

                info!("Semantic cache HIT (native): similarity={:.3}, time={:?}", similarity, start.elapsed());
    ...
    async fn get_similar_fallback(&self, request: &ChatRequest, query_embedding: Vec<f32>, cutoff_time: chrono::DateTime<chrono::Utc>) -> Result<Option<CachedResponse>> {
        ...
        while let Some(row) = rows.next().await? {
            let cached_embedding: Vec<f32> = row.get::<Vec<u8>>(2)?
                .chunks_exact(4)
                .map(|bytes| f32::from_le_bytes([bytes[0], bytes[1], bytes[2], bytes[3]]))
                .collect();
            ...
            if similarity >= self.config.similarity_threshold {
                if best_match.as_ref().map_or(true, |(s, _)| similarity > *s) {
                    ...
                    let timestamp = chrono::DateTime::parse_from_rfc3339(&row.get::<String>(1)?)?.with_timezone(&chrono::Utc);
                    
                    best_match = Some((similarity, CachedResponse {
                        response,
                        timestamp,
                        embedding: Some(cached_embedding),
                    }));
    ...
    async fn cleanup_old_entries(&self) -> Result<()> {
        let cutoff_time = chrono::Utc::now() - chrono::Duration::hours(self.config.ttl_hours as i64 * 2);
